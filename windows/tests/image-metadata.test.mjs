...
const featuredMetadata = readImageMetadata(featured, ".jpg");
assert.ok(featuredMetadata, "The default development background must be a valid image.");
assert.equal(featuredMetadata.width, 1920);
assert.equal(featuredMetadata.height, 1200);
assert.equal(featuredMetadata.aspect, "wide");
assert.equal(featuredMetadata.taskMode, "ambient");

const cli = spawnSync(process.execPath, [helper, "--check", path.join(windowsRoot, "assets", "dream-reference.jpg")], {
  encoding: "utf8",
});
assert.equal(cli.status, 0);
assert.deepEqual(JSON.parse(cli.stdout), featuredMetadata);
