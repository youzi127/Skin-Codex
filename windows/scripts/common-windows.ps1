...
function Get-DreamSkinDefaultStateRoot {
  $stateRoot = Join-Path -Path $env:LOCALAPPDATA -ChildPath 'SkinCodex'
  return $stateRoot
}

function Enter-DreamSkinOperationLock {
  $sid = [System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value
  $mutex = [System.Threading.Mutex]::new($false, "Local\SkinCodex.$sid.Operation")
  $acquired = $false
  try {
  ...
  if (-not $acquired) {
    $mutex.Dispose()
    throw '另一个 Skin Codex 安装、启动、恢复或验证操作正在运行。'
  }
  return $mutex
}
...
function Test-DreamSkinPathEqual {
  ...
  try {
    ...
    $leftFull = [System.IO.Path]::GetFullPath($Left).TrimEnd($separator)
    $rightFull = [System.IO.Path]::GetFullPath($Right).TrimEnd($separator)
...
function Get-DreamSkinRuntimeEnginePaths {
  ...
  $root = Join-Path -Path $fullStateRoot -ChildPath 'engine'
  ...
  return [pscustomobject]@{
    ...
    HiddenLauncher = Join-Path $scripts 'launch-hidden.vbs'
  }
}

function Test-DreamSkinTrayActive {
  $sid = [System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value
  foreach ($mutexName in @("Local\SkinCodex.$sid.Tray", "Local\NovaSkin.$sid.Tray")) {
...
function Stop-DreamSkinTrayProcess {
  ...
  $trayScript = [System.IO.Path]::GetFullPath((Join-Path $ScriptRoot 'tray-dream-skin.ps1'))
  try {
    ...
    foreach ($process in $processes) {
      ...
      $isCurrentTray = $process.CommandLine.IndexOf($trayScript, [System.StringComparison]::OrdinalIgnoreCase) -ge 0
      $isInstalledTray = $process.CommandLine.IndexOf('SkinCodex\scripts\tray-dream-skin.ps1', [System.StringComparison]::OrdinalIgnoreCase) -ge 0
      $isRuntimeTray = $process.CommandLine.IndexOf('SkinCodex\engine\scripts\tray-dream-skin.ps1', [System.StringComparison]::OrdinalIgnoreCase) -ge 0
      $isLegacyInstalledTray = $process.CommandLine.IndexOf('NovaSkin\scripts\tray-dream-skin.ps1', [System.StringComparison]::OrdinalIgnoreCase) -ge 0
      $isLegacyRuntimeTray = $process.CommandLine.IndexOf('NovaSkin\engine\scripts\tray-dream-skin.ps1', [System.StringComparison]::OrdinalIgnoreCase) -ge 0
      if ($isCurrentTray -or $isInstalledTray -or $isRuntimeTray -or $isLegacyInstalledTray -or $isLegacyRuntimeTray) {
        Stop-Process -Id $process.ProcessId -Force -ErrorAction Stop
      }
    }
  } catch {
    Write-Warning "无法自动关闭 Skin Codex 托盘：$($_.Exception.Message)"
  }
}

function Install-DreamSkinShortcutIcon {
  param(
    [Parameter(Mandatory = $true)][string]$SourceIconPath,
    [Parameter(Mandatory = $true)][string]$StateRoot
  )
  $source = [System.IO.Path]::GetFullPath($SourceIconPath)
  if (-not (Test-Path -LiteralPath $source -PathType Leaf)) {
    throw "Skin Codex shortcut icon does not exist: $source"
  }
  $fullStateRoot = [System.IO.Path]::GetFullPath($StateRoot)
  Ensure-DreamSkinManagedDirectory -Path $fullStateRoot -Root $fullStateRoot
  $hash = (Get-FileHash -Algorithm SHA256 -LiteralPath $source).Hash.ToLowerInvariant().Substring(0, 12)
  $target = Join-Path $fullStateRoot "skin-codex-$hash.ico"
  if (-not (Test-DreamSkinPathEqual -Left $source -Right $target)) {
    Copy-Item -LiteralPath $source -Destination $target -Force
  }
  $stableIcon = Join-Path $fullStateRoot 'skin-codex.ico'
  if (-not (Test-DreamSkinPathEqual -Left $source -Right $stableIcon)) {
    Copy-Item -LiteralPath $source -Destination $stableIcon -Force
  }

  Get-ChildItem -LiteralPath $fullStateRoot -Filter 'skin-codex-*.ico' -File -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -ine $target } |
    Remove-Item -Force -ErrorAction SilentlyContinue
  Get-ChildItem -LiteralPath $fullStateRoot -Filter 'nova-skin-*.ico' -File -ErrorAction SilentlyContinue |
    Remove-Item -Force -ErrorAction SilentlyContinue
...
function Install-DreamSkinRuntimeEngine {
  ...
  $sourceDirectories = @('scripts', 'assets')
  if (Test-Path -LiteralPath (Join-Path $sourceRoot 'bundled-themes') -PathType Container) {
    $sourceDirectories += 'bundled-themes'
  }
  ...
  try {
    ...
    $sourceFiles = @(
      Get-ChildItem -LiteralPath ($sourceDirectories | ForEach-Object { Join-Path $sourceRoot $_ }) `
        -Recurse -File -Force -ErrorAction Stop
    )
    $stagedFiles = @(
      Get-ChildItem -LiteralPath ($sourceDirectories | ForEach-Object { Join-Path $stagingRoot $_ }) `
        -Recurse -File -Force -ErrorAction Stop
    )
...
function Get-DreamSkinNodeRuntime {
  ...
  $pathProbe = Invoke-DreamSkinNative -FilePath $nodePath -ArgumentList @('-p', 'process.execPath') -DiscardStderr
...
function Read-DreamSkinState {
  ...
  try {
    ...
    if ($schemaVersion -gt 3) { throw 'State schema is not supported.' }
...
function Confirm-DreamSkinRestart {
  ...
  return $shell.Popup($Message, 0, 'Skin Codex', 52) -eq 6
}
