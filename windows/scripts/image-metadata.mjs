...
const sourcePath = typeof __filename === "string" ? __filename : fileURLToPath(import.meta.url);
const isCommonJsMain = typeof require === "function" && typeof module === "object" && require.main === module;
const isEsModuleMain = typeof require !== "function" && process.argv[1] &&
  path.resolve(process.argv[1]) === path.resolve(sourcePath);
if (isCommonJsMain || isEsModuleMain) {
  main().catch((error) => {
    console.error(error?.message ?? String(error));
    process.exitCode = 2;
  });
